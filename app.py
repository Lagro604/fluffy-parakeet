...
TRADE_THRESHOLD = 20000000  # 2000만원
EXCLUDED_TRADE_THRESHOLD = 70000000  # 7000만원
EXCLUDED_COINS = ['KRW-SOL', 'KRW-ETH', 'KRW-SHIB', 'KRW-DOGE', 'KRW-USDT', 'KRW-XRP']  # 예외로 처리할 코인
        ...
        for market_id, coin_name in COIN_NAMES.items():
            ...
            if isinstance(recent_trades, list) and recent_trades:
                ...
                if market_id in EXCLUDED_COINS:
                    trade_found_excluded = any(trade['trade_price'] * trade['trade_volume'] >= EXCLUDED_TRADE_THRESHOLD for trade in recent_trades)
                    if trade_found_excluded:
                        message = f"{coin_name} (제외 코인)에서 7,000만원 이상의 체결이 발생하였습니다."
                        msg_id = hashlib.md5(message.encode()).hexdigest()
                        if msg_id not in recent_messages:
                            await send_telegram_message(message)
                            recent_messages.add(msg_id)
                elif trade_found:
                    message = f"매수 알림: {market_id} ({coin_name})\n최근 거래 중 2,000만원 이상의 체결이 발생하였습니다."
                    msg_id = hashlib.md5(message.encode()).hexdigest()
                    if msg_id not in recent_messages:
                        await send_telegram_message(message)
                        recent_messages.add(msg_id)
