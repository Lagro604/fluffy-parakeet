...
def fetch_upbit_data():
    ...
    logger.error(f"업비트 데이터 요청 실패: {response.status_code}")
    return []

# 바이낸스 데이터 요청 함수
def fetch_binance_data():
    url = "https://api.binance.com/api/v3/aggTrades?symbol=BTCUSDT&limit=10"
    response = requests.get(url)
    if response.status_code == 200:
        return response.json()
    logger.error(f"바이낸스 데이터 요청 실패: {response.status_code}")
    return []
...
def check_trades():
    while True:
        ...
        for trade in upbit_data:
            ...
            if trade_value >= UPBIT_CONDITIONS['high_value']:
...
def webhook():
    ...
    if request.method == 'POST':
        ...
        return 'GET 요청 수신!', 200  # GET 요청에 대한 응답
